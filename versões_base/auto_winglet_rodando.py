...
# Tenho 3 funções mas os inputs entram na primeira e os outputs saem na ultima, foda..clear

def AVL(Altura, Enflechamento, Cordap, Envergadura):
...
class WingletOptimization(Problem):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        for i in range(n):
            # Avalia os objetivos para o indivíduo i
            CD, CL, Cn, Cl = AVL(X[i, 0], X[i, 1], X[i, 2], X[i, 3])

            # Atribui os valores dos objetivos ao indivíduo i na matriz F
            F[i, 0] = CD
            F[i, 1] = CL
            F[i, 2] = Cn
            F[i, 3] = Cl
...
algorithm = RNSGA2(
    ...
    epsilon=0.01,
    normalization='front',
    weights = np.array([[0.6, 0.5, 0.5, 0.35]])
)
...
print(historico)
CL              = objective_values[:, 0]
CD              = objective_values[:, 1]
Cn              = objective_values[:, 2]
...
# Plot
plt.figure()
plt.scatter(altura, CD, marker= "x",label='altura')
plt.xlabel('ALTURA DO WINGLET')
plt.ylabel('CD')
plt.legend()
plt.show()

plt.scatter(enflechamento, CD, marker= "x",label='enflechamento')
plt.xlabel('enflechamento DO WINGLET')
plt.ylabel('CD')
plt.legend()
plt.show()

plt.scatter(cordap, CD, marker= "x",label='CORDA NA PONTA')
plt.xlabel('CORDA NA PONTA DO WINGLET')
plt.ylabel('CD')
plt.legend()
plt.show()

plt.scatter(envergadura, CD, marker= "x",label='ENVERGADURA')
plt.xlabel('ENVERGADURA DO WINGLET')
plt.ylabel('CD')
plt.legend()
plt.show()
...
# Plot
plt.figure()
plt.scatter(altura, Cn, marker= "x",label='altura')
plt.xlabel('ALTURA DO WINGLET')
plt.ylabel('Cn')
plt.legend()
plt.show()

plt.scatter(enflechamento, Cn, marker= "x",label='enflechamento')
plt.xlabel('enflechamento DO WINGLET')
plt.ylabel('Cn')
plt.legend()
plt.show()

plt.scatter(cordap, Cn, marker= "o",label='cordap')
plt.xlabel('cordap DO WINGLET')
plt.ylabel('Cn')
plt.legend()
plt.show()

plt.scatter(envergadura, Cn, marker= "x",label='envergadura')
plt.xlabel('envergadura DO WINGLET')
plt.ylabel('Cn')
plt.legend()
plt.show()
...
# Plot
plt.figure()
plt.scatter(altura, CL, marker= "x",label='altura')
plt.xlabel('ALTURA DO WINGLET')
plt.ylabel('CL')
plt.legend()
plt.show()

plt.scatter(enflechamento, CL, marker= "x",label='enflechamento')
plt.xlabel('enflechamento DO WINGLET')
plt.ylabel('CL')
plt.legend()
plt.show()

plt.scatter(cordap, CL, marker= "x",label='cordap')
plt.xlabel('cordap DO WINGLET')
plt.ylabel('CL')
plt.legend()
plt.show()

plt.scatter(envergadura, CL, marker= "x",label='envergadura')
plt.xlabel('envergadura DO WINGLET')
plt.ylabel('CL')
plt.legend()
plt.show()